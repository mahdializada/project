...
class CompanyExport extends DefaultValueBinder implements
    WithEvents,
    WithHeadings,
    shouldAutoSize,
    withMultipleSheets,
    ...
    WithColumnFormatting
{
    ...
    protected $columns = ['D', 'E', 'F'];
    protected $investment_type = [];
    ...
    {
        $sheets = [];
        $sheets['companies'] = new CompanyExport($this->investment_type, $this->init_status);
        $sheets['systems'] = new SystemSheetExport();
                    ...
                    for ($i = 0; $i < count($this->column_name); $i++) {
                        ...
                        switch ($this->column_name[$i]) {
                            case 'Investment_type': {
                                    $objValidation->setFormula1('"' . implode(', ', $this->investment_type) . '"');
                                }
                                break;
                            default: {
    ...
    public function styles(Worksheet $sheet): array
    {
        ...
        $sheet->setCellValue('A4', 'Company Bulk Upload Template');
        $alignment = [
