...
//use Your Model
use App\Models\SingleSales\Attachment;
use Exception;
        ...
        try {
            ...
            if ($request->hasFile('logos')) {
                ...
                foreach ($files as $index => $file) {
                    ...
                    $new['name'] = $request->brands[$index];
                    $new['arabic_name'] = $request->arabic_name[$index];
                    $attrModel = Brand::create($new);


                    // $name = $file->extension();
                    $uploadedPath = $file->store($path);
                    $attrModel->attachment()->create(['path' => $uploadedPath, 'attachmentable_id' => $attrModel->id]);
