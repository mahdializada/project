...
class AdvertisementUtilMU
{
    ...
    public static function getStatus($status)
    {
        if ($status) {
            ...
            return collect($statuses)->get($status) ?? $status;
        }
        return $status;
    }
    ...
    {
        try {
            $connections = ConnectionMURepository::connectionAccount();
            $items       = [];
    ...
    public static function countAndAdsCalculations(
        ...
        $query = AdsMU::getAdsCalculations($query, $request, $group_by, $relations);
        $query = AdsMU::getObjectTotals($query, $removed_columns, $request);
        $query = AdvertisementUtilMU::filterByQueryIds($query, $request, $filter_id);
        $query = AdvertisementUtilMU::filterConnectionParams($query, $request);
        return $query;
    }
    ...
    public static function filterConnectionParams($query, Request $request, $relation_name = "connections")
    {
        return $query->whereHas("$relation_name", function ($con) use ($request) {
            ...
            if ($request->item_code && $type != "item_code") {
                $item_codes = ConnectionMU::whereIn("id", $request->item_code)->select("pcode")->get()->toArray();
    ...
    public static function expireApplication(Application $application)
    {
        ...
        $users = [["name" => "Asadullah Moradi", 'phone' => "+93748485799"], ['name' => "Omar AlMurshid", "phone" => "+971562876382"]];
    ...
    public static function searchableColumns($newcolumns = [])
    {
        $defaultColumns = [
            ...
            "ga_user_engagement", "ga_sessions", "ga_engaged_sessions", "ga_page_views", "cpm", "ctr", "cpc",
