    ...
    >
      ...
      <template v-slot:fileUpload>
        ...
        >
          <template v-slot:[`item.name`]="{ item, index }">
            <div class="w-full">
              ...
              >
                ...
                >
                  <span class="me-1 px-3">{{
                    item.name ? item.name : $tr("empty")
                  }}</span>
                  <v-progress-circular
                    v-if="
                      checkIfHasErr(item.name, 'name', index + 6) &&
          ...
          <template v-slot:[`item.industry`]="{ item, index }">
            <div>
              <v-edit-dialog
                ...
                large
                @open="onOpen(item.industry, 'industry_id')"
                @save="
                  onSave('industry_id', 'industry', index + 6, department.industry_id)
                "
              >
                <div
                  :class="`${
                ...
                >
                  {{ item.industry ? item.industry : $tr("empty") }}
                </div>
                <template v-slot:input>
                  <CustomInput
                    class="mt-3"
          ...
          <template v-slot:[`item.business_location`]="{ item, index }">
            <div>
              <v-edit-dialog
                :return-value.sync="item.business_location"
                large
                @open="onOpen(item.business_location, 'business_location_id')"
                @save="
                  onSave(
                    'business_location_id',
                    'business_location',
                    index + 6,
                    department.business_location_id
                  )
                "
              >
                <div
                  :class="`${
                    checkIfHasErr(item.business_location, 'business_location', index + 6)
                      ? 'text--white error-class px-5 d-flex align-center justify-center'
                      : ''
                  }`"
                >
                  {{ item.business_location ? item.business_location : $tr("empty") }}
                </div>
                <template v-slot:input>
                  <CustomInput
                    class="mt-3"
                    ...
                    item-value="name"
                    item-text="name"
                    :label="$tr('label_required', $tr('business_locations'))"
                    :placeholder="$tr('choose_item', $tr('business_locations'))"
                    type="autocomplete"
                    v-model="$v.department.business_location_id.$model"
                    :rules="rules.business_locationRule($v.department, $root)"
                  />
                </template>
              </v-edit-dialog>
            </div>
          ...
          <template v-slot:[`item.company`]="{ item, index }">
            <div>
              <v-edit-dialog
                ...
                large
                @open="onOpen(item.company, 'company_ids')"
                @save="
                  onSave('company_ids', 'company', index + 6, department.company_ids)
                "
              >
                <div
                  :class="`${
                ...
                <template v-slot:input>
                  <CustomInput
                    ...
                    company
                    type="autocomplete"
                    v-model="$v.department.company_ids.$model"
        ...
        <v-simple-table dense v-show="errors.length > 0">
          <template v-slot:default>
            ...
            <tbody>
              <tr v-for="(error, index) in errors" :key="index" class="custom-row-error">
      ...
      <template v-slot:done>
        <div class="d-flex justify-center">
          <done-message
            :title="$tr('item_all_set', $tr('departments'))"
            :subTitle="$tr('you_can_access_your_item', $tr('departments'))"
          />
        </div>
      </template>
    </CustomStepper>
...
export default {
  ...
  data() {
    return {
      ...
      selectedHeaders: [
        ...
        {
          text: this.$tr("industry"),
          value: "industry",
        },
        {
          text: this.$tr("business_location"),
  ...
  validations: {
    department: {
      ...DepartmentValidation.validations.department,
  ...
  methods: {
    onOpen(value, col) {
      this.$v.department[col].$model = value;
    },
    async onSave(key, col, index, value) {
      let dublicable_columns = ["note", "company", "industry", "business_location"];
    ...
    resetForm() {
      ...
      this.$v.form.$reset();
      this.$refs.department_bulk_upload_file_form?.reset();
      this.$refs.department_bulk_upload_file_form?.resetValidation();
    ...
    async readFileAsXlsx() {
      readXlsxFile(this.form.file).then(async (rows) => {
        ...
        let is_correct_form_uploaded = selectedHeaders.every((h) =>
    ...
    async prepareMainValidations(selectedHeaders, selectedRows) {
      ...
      await this.selectedRows.forEach(async (row, counter) => {
        ...
        this.percentage =
          this.percentage + Number.parseInt(100 / this.selectedRows.length);
      });
    },
    ...
    async validateSelectedRows(rows, selectedHeaders) {
      ...
      rows.forEach((elements, counter) => {
        elements.forEach((col, index) => {
          ...
          if (key === "company") {
            this.$v.department.company_ids.$model = col;
          } else if (key === "industry") {
            this.$v.department.industry_id.$model = col;
          } else if (key == "business_location") {
            this.$v.department.business_location_id.$model = col;
          } else {
  ...
  async mounted() {
    ...
    try {
      if (this.companies.length == 0) {
        res = await this.$axios.get(`companies?key=only-companies`);
        this.companies = res?.data?.companies;
      }
      if (this.business_locations.length == 0) {
        res = await this.$axios.get(`business-locations?key=only-business-locations`);
        this.business_locations = res?.data?.data;
      }
      res = await this.$axios.get("industries");
