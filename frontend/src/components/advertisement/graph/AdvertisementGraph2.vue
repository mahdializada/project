<template>
  <div class="d-flex">
    <SerriesCard
      @toggleColumn="toggleSerries($event)"
      @onSubmit="onSubmit"
      :series_type="series_type"
      ref="serriesRef"
      :style="` ${isLarg ? 'min-width: 18%' : 'min-width: 22%'}`"
    />
    <div
      class="pa-2 pb-1 px-1"
      :style="` ${isLarg ? 'min-width: 82%' : 'min-width: 78%'}`"
    >
      <v-card min-height="400" elevation="0" class="white pt-1">
        <div class="d-flex align-start justify-space-between">
          ...
          >
            <v-chip
              ...
              :key="index"
              class="me-1 mb-1"
              :style="` height: 25px ;background-color: ${item.backgroundColor.replace(
                '1)',
                '0.08)'
          ...
          <div class="ps-1 pe-2" v-if="series_type == 'advertisement'">
            <v-select
              on
        ...
        <div id="legend-container" class="position-relative pa-2">
  ...
  methods: {
    toggleSerries(params) {
      ...
      if (action == "push") {
        ...
        if (action == "manual-pop") {
          ...
          this.$refs.serriesRef.unselectSerries(label);
        } else {
          this.chartData.datasets = [
    ...
    async applySavedFilter(id, labels) {
      ...
      for (let index = 0; index < data.length; index++) {
        const temp_data = data[index];
        let dataset = JSON.parse(JSON.stringify(this.defaultDataSet));
        dataset.label = this.$tr(serries[index].label);
        dataset.data = temp_data.data;
        dataset.backgroundColor = serries[index].color;
        dataset.borderColor = serries[index].color;
        this.chartData.datasets.push(dataset);
        this.chartData.labels = temp_data.label;
      }
      this.loading_graph_api = false;
