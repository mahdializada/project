<template>
  ...
  <div
    id="my-overlay"
    :class="`${overlay ? 'overlay' : ''} ${!show ? 'hide' : 'show'}`"
  >
    <div
      :class="`graph-container  ${!show ? 'hide' : 'show'} ${
    ...
    >
            ...
            >
              ...
              <div class="mb-5">
                ...
                <div
                  ...
                  "
                >
                  <div v-if="remark_data.length > 0" class="pe-1">
                    <single-remark
                        ...
                        >
                          ...
                          <template v-slot:body>
                            <div>
                              {{
                                $tr(
                                  "please_wait_30_seconds_before_your_next_remark"
                                )
                              }}
                            </div>
                          </template>
      ...
      <v-scroll-x-transition>
        <div v-show="selected_profile == 1">
          <advertisement-profile-percentage
      ...
      <v-scroll-x-transition>
        <div v-show="selected_profile == 2">
          <AdvertisementProfileTarget
            @closeProfile="showProfile"
            :current_tab.sync="current_tab"
            :selected_item.sync="selected_item"
            @fullScreen="fullScreen = !fullScreen"
            ref="profileTargetRefs"
          />
        </div>
      </v-scroll-x-transition>
...
import ProfileHorizentalTab from "./ProfileHorizentalTab.vue";
import AdvertisementProfilePercentage from "./AdvertisementProfilePercentage.vue";
import AdvertisementProfileTarget from "./AdvertisementProfileTarget.vue";
...
export default {
  ...
  methods: {
    onTabChange(index) {
      ...
      if (index == 2) {
        this.$refs.profileTargetRefs.showProfile();
      }
    ...
    async showProfile(item = {}) {
      if (!this.show) {
        ...
        try {
          this.$refs.profileTargetRefs.resetProfile();
        } catch (e) {}
      }
    },
