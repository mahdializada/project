<template>
  ...
  <v-card
    height="470"
    elevation="0"
    style="min-height: 470px; max-height: 470px"
    class="pa-1 py-2 pb-3 d-flex flex-column align-center"
  >
    <v-text-field
      hide-details
      v-model="search"
    ...
    >
      <v-list rounded dense color="" class="pb-2 ps-0">
        <v-list-item-group color="primary" multiple v-model="selected_serries">
          <v-list-item
    ...
    >
      ...
      <div class="pa-2 white">
        <p class="mb-1" style="font-size: 16px; font-weight: 500; color: #777">
          {{ $tr("view_name") }}
        </p>
        <v-text-field
...
export default {
  data() {
    return {
      ...
      loading: false,
      serries: [
        { label: "spend", color: "rgba(3 ,155, 229,1)" },
  ...
  methods: {
    toggleColumn(item) {
      ...
      if (this.selected_serries.includes(item.label)) {
        ...
        this.$root.$emit("toggleSerries", { label: item, action: "pop" });
      } else {
        this.selected = this.selected.push(item.label);
        this.$root.$emit("toggleSerries", { label: item, action: "push" });
      }
    },
    ...
    assignSerries(data) {
      console.log("serriesddddddddddd", data);
      this.selected_serries = data;
      const serr = this.serries.filter((row) => data.includes(row.label));
      this.$root.$emit("assignedResult", serr);
      return serr;
    },
    async finalSubmit() {
  ...
  mounted() {
    this.$root.$on("unselectSerries", this.unselectSerries);
    this.$root.$on("assignSerries", this.assignSerries);
  },
  created() {},
};
</script>
