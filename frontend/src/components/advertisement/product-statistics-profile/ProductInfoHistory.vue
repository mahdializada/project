<template>
  <v-dialog v-model="showDialog" width="800" persistent>
    <v-card class="white">
      <v-card-title class="pa-2 pb-5" style="color: #777">
        <span class="dialog-title d-inline-block ps-1">
          {{ $tr("history") }}</span
        >
        <v-spacer />
        <div class="d-flex">
      ...
      <div v-if="!loading" style="height: 400px; overflow-y: auto" class="me-2">
        <v-timeline align-top dense flat class="customTimeLine3 pt-0 pe-1">
          ...
          >
            ...
            <div>
              <div
                class="text-body-1 mb-1 font-weight-regular d-flex justify-space-between"
              >
                <span class="">
        ...
        <NoRemark
          v-if="(histories.length == 0 || histories == null) && !loading"
          style="position: absolute; top: 30%; left: 34%"
        />
      </div>
...
export default {
  components: { SvgIcon, FilterDateRange, NoRemark },
  data() {
    return {
      key: 0,
      column: "",
