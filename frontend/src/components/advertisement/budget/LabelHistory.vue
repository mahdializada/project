		...
		>
			<template v-if="!skeleton_loading">
				<v-timeline
					...
					align-top
					v-if="!(labels_data.length === 0)"
				>
					<!-- (Object.keys(labels_data).length === 0) -->
					<v-timeline-item
						v-for="(history, key) in labels_data"
						:key="key"
						color="primary"
						icon="mdi-book"
					>
						...
						<div class="d-flex flex-wrap py-4">
							<div
								...
								v-for="labelable in history"
								:key="labelable.id"
							>
								<div
									class="label-color me-1 white--text text-center text-uppercase"
									:style="'background-color:' + labelable.label.color"
								>
									{{ labelable.label.label.charAt(0) }}
								</div>
...
export default {
	...
	created() {
		let data = { tab: this.data.model, id: this.getId() };
	...
	methods: {
		async getLabels(data) {
			try {
				...
				this.labels_data = response.data;
				// this.has_label = this.labels_data.filter(
				// 	(row) => row.label.length > 0
				// );
				// let items = this.faltObject(this.labels_data);
				// this.labels_data = this.groupBy(items, "id");
		...
		getId() {
			switch (this.data.model) {
				case "ad":
					return this.data.item?.ad_id;
				case "ad_set":
					return this.data.item?.adset_id;
				case "campaign":
					return this.data.item?.campaign_id;
				default:
