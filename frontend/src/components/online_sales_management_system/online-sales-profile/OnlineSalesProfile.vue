  ...
  >
    ...
    >
      <v-scroll-x-transition>
        <div class="h-full" v-show="selected_profile == 'details'">
                    ...
                    <v-avatar size="40" class="grey lighten-3 me-1">
                      <v-avatar size="35" color="white">
                        <v-avatar size="30" color="white">
                          ...
                          >
                            {{ selected_item.product_code[0] }}
                          </v-avatar>
                        </v-avatar>
                      </v-avatar>
                    </v-avatar>
                    <span class="dialog-title">
                      {{ selected_item.product_code }}</span
                    >
                  </span>
                </div>
                <div class="d-flex align-center">
                  <slot name="date_slot">
                    <FilterDateRange
                      ref="filterDateRange"
                      :customSelectDate="{ type: 'today', index: 0 }"
                      :dateRangeProp="date_range_0"
                      :hide-title="true"
      ...
      <!-- Peofile Note -->
      <v-scroll-x-transition>
        <div class="h-full" v-show="selected_profile == 'note'">
          <OnlineSalesNote
      ...
      <v-scroll-x-transition>
        <div class="h-full" v-show="selected_profile == 'item_status_history'">
          <ItemStatusHistory
            ref="itemStatusHistoryRef"
            :selectedItems="selected_item"
            :selectedDate="date_range"
            @fullScreen="fullScreen"
            @closeDialog="closeDialog"
            :openItemProfile="openItemProfile"
            @changeStatus="$emit('changeStatus')"
          />
        </div>
      </v-scroll-x-transition>

      <!-- end item status history -->
      <!-- design request -->
      <v-scroll-x-transition>
        <OnlineSalesDesignRequest
          ref="designRequestRefs"
          :selected_product="selected_item"
          v-show="selected_profile == 'design_request'"
          @closeDialog="closeDialog()"
          @fullScreen="fullScreen()"
        />
      ...
      <v-scroll-x-transition>
        <OnlineSalesGraph
          v-show="selected_profile == 'graph'"
          ref="onlineSalesGraphRefs"
          :selected_product="selected_item"
...
import AdvertisementGraphProfileProduct from "../../advertisement/graph/AdvertisementGraphProfileProduct.vue";
import ColumnChart from "../../advertisement/product-statistics-profile/columnChart.vue";
import LandingPageMedia from "../../advertisement/product-statistics-profile/LandingPageMedia.vue";
import LandingPagePreview from "../../advertisement/product-statistics-profile/LandingPagePreview.vue";
import ProductStatisticsGeneralInfo from "../../advertisement/product-statistics-profile/ProductStatisticsGeneralInfo.vue";
import ProductTotalInfoCard from "../../advertisement/product-statistics-profile/ProductTotalInfoCard.vue";
import SalesCard from "../../advertisement/product-statistics-profile/SalesCard.vue";
import ProfileHorizentalTab2 from "../../advertisement/graph/ProfileHorizentalTab2.vue";
import CustomButton from "../../design/components/CustomButton.vue";
import NoteName from "./NoteName.vue";

import ItemStatusHistory from "./ItemStatusHistory.vue";
import OnlineSalesGraph from "./OnlineSalesGraph.vue";
import OnlineSalesDesignRequest from "./OnlineSalesDesignRequest.vue";
import OnlineSalesNote from "./OnlineSalesNote.vue";
...
export default {
  ...
  data() {
    return {
      ...
      showDialog: false,
      profile_data: {},
      selected_item: { product_code: "R", pname: "pr30" },
  ...
  computed: {
    ...
    tabItems() {
      if (this.current_tab == "item_code") {
        this.selected_profile = "details";
        return [
          "details",
          "note",
          "item_status_history",
  ...
  methods: {
    ...
    async onTabChange(index) {
      ...
      if (index == "item_status_history") {
        this.$refs.itemStatusHistoryRef.fetchData();
      }

      if (index == "design_request") {
        this.$refs?.designRequestRefs?.fetchDesignRequests();
      }

      if (index == "graph") {
        this.$refs?.onlineSalesGraphRefs?.fetchExtraInfo(this.selected_item);
      }
      this.date_range_0 = {
        start_date: moment().format("YYYY-MM-DD"),
        end_date: moment().format("YYYY-MM-DD"),
    ...
    async openDialog(item) {
      this.openItemProfile = true;
      try {
        this.init_product_graph = true;
        this.showDialog = true;
        this.selected_item = await JSON.parse(JSON.stringify(item));
        ...
        if (this.current_tab != "item_code") {
          this.selected_profile == "graph";
          this.$refs?.onlineSalesGraphRefs?.fetchExtraInfo(item);
        } else {
    ...
    async fetchProfileData() {
      try {
        ...
        const response = await this.$axios.get(
          ...
          { params: { item_code: this.selected_item.product_code } }
        );
