    ...
    <client-only>
      ...
      >
            ...
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                <v-img
                  ...
                  height="30"
                  lazy-src="https://picsum.photos/id/11/10/6"
                  :src="replaceText(item?.attachment[0]?.path)"
                ></v-img>
              </span>
            </template>
            <span>
        ...
        <template v-slot:[`item.icon`]="{ item }">
          <!-- <v-menu offset-y open-on-hover :close-on-content-click="false">
            <template v-slot:activator="{ on, attrs }">
              <span style="white-space: nowrap" v-bind="attrs" v-on="on" class="mb-1">
                <v-img v-on:mouseover="previewLogo" v-on:mouseleave="leaveMouse" class="rounded-circle" width="30"
                  height="30" lazy-src="https://picsum.photos/id/11/10/6" :src="item.icon"></v-img>
              </span>
            </template>
            <span>
              <v-img v-if="isPreview" width="400" lazy-src="https://picsum.photos/id/11/10/6" :src="item.icon"></v-img>
            </span>
          </v-menu> -->
          <template>
        ...
        <template v-slot:[`item.banner`]="{ item }">
          <!-- <v-menu offset-y open-on-hover :close-on-content-click="false">
            <template v-slot:activator="{ on, attrs }">
              <span style="white-space: nowrap" v-bind="attrs" v-on="on" class="mb-1">
                <v-img v-on:mouseover="previewLogo" v-on:mouseleave="leaveMouse" class="rounded-circle" width="30"
                  height="30" lazy-src="https://picsum.photos/id/11/10/6" :src="item.banner"></v-img>
              </span>
            </template>
            <span>
              <v-img v-if="isPreview" width="400" lazy-src="https://picsum.photos/id/11/10/6" :src="item.banner">
              </v-img>
            </span>
          </v-menu> -->
    ...
    <v-row class="pb-3 align-center ma-0 py-2 tbl-bottom" v-if="hasPagination">
      ...
      <v-col class="py-0" cols="12" :md="paginateSmall ? '5' : '4'">
        ...
        >
          ...
          <div style="width: 90px !important; margin: 0.16rem 0.16rem;z-index: 1;">
            <v-select
...
export default {
  ...
  watch: {
    ...
    options: function (options) {
      this.currentIndex = options.page;
    },
  },
  ...
  methods: {
    ...
    perPageSelect() {
      ...
      console.log("perPageSelect", this.options);
    },
    ...
    replaceText(item) {
      return "http://localhost:8000/" + item?.replace("public/", "storage/");
    },
    setPaginationTo(page) {
      this.options = { ...this.options, page: page };
    },
    setPerPageTo(perpage) {
      this.itemsPerPage2 = perpage;
      this.options = { ...this.options, itemsPerPage: perpage };
    },
      ...
      if (index.length !== 0) {
        ...
        if (myArray.length > 1) {
          items = items.sort((a, b) => {
            a = isNaN(parseFloat(a[myArray[0]][myArray[1]]))
              ? 0
              : parseFloat(a[myArray[0]][myArray[1]]);
            b = isNaN(parseFloat(b[myArray[0]][myArray[1]]))
              ? 0
              : parseFloat(b[myArray[0]][myArray[1]]);
