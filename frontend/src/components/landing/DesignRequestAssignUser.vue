  ...
  methods: {
    ...
    async fetchTeamUsers(selected_teams) {
      try {
        ...
        const response = await this.$axios2.post("users/selected-companies", {
          permission: "My Order",
          team_ids: selected_teams.map((t) => t.id),
        });

        this.userLoading = false;
        return this.prepareUsersAccordingToPermission(
          response?.data?.users ?? []
        );
      } catch (err) {
        this.userLoading = false;
    ...
    async fetchTeams() {
      ...
      try {
        ...
        const url = "teams/selected-companies";
        const response = await this.$axios2.post(url, {
          permission: "My Order",
        });
