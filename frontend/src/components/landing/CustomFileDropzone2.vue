<template>
  ...
  <div v-else class="dropzone mt-2">
    <input
      ...
      ref="fileInputRef"
      multiple
      accept="application/pdf , .rar , .pdf , .doc , .docx , application/msword , application/vnd.openxmlformats-officedocument.wordprocessingml.document , .zip , application/x-zip-compressed , image/png , image/jpeg , image/jpg"
    ...
    <div
      ...
      @click="skipDD ? () => {} : $refs.fileInputRef.click()"
      @dragover="dragover"
      @drop="dropFiles"
    >
      <div class="dropzone__text">
        <div class="d-flex align-center flex-column">
          <div v-if="!skipDD" class="dropzone__button">
            <v-btn
          ...
          <div class="ms-1 dropzone__text-title">
            {{ !skipDD ? $tr("drag_and_drop") : $tr("you_have_skipped") }}
          </div>
        </div>
        <div v-if="!skipDD">
          {{ $tr("allowed_extensions") }}
          <p class="mb-0">
            {{ $tr("pdf, .doc, .docx, .png, .jpeg, .jpg, .zip") }}
          </p>
        </div>
      </div>
      <v-btn
        v-if="skipBtn"
        class="ms-auto skipCss"
        @click.stop="!skipDD ? skip() : Cancelskip()"
        outlined
        color="primary"
        depressed
        >{{ !skipDD ? $tr("skip") : $tr("cancel_skip") }}</v-btn
      >
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      isLoading: false,
      file_uploadable_stages: ["in design", "in programming"],
      skipDD: false,
    };
  },
  ...
  methods: {
    authUploadFile() {
      return (
        this.$isInScope("drva") &&
        this.file_uploadable_stages.includes(this.status)
      );
    },
    onFileDelete(file) {
      const app = this;
    ...
    async onFileInputChange(event) {
      ...
      for (let fileIndex = 0; fileIndex < uploadedFiles.length; fileIndex++) {
        ...
        // if (this.allowedExtensions && !allowedExtensions.includes(fileExtension)) {
        ...
        if (this.fileSize && (file.size / 1024).toFixed(1) > this.fileSize) {
          this.isLoading = false;
          this.errorTexts = this.$tr("file_max_size_error", 2);
          return;
        }
