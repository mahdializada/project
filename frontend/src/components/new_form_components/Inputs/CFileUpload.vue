...
import CFileItems from "./CFileItems.vue";
  ...
  methods: {
    ...
    async upload(file) {
      try {
        ...
        formData.append("path", this.storePath);
        ...
        if (this.isUpload) {
          ...
          formData.append("type", this.form.check_type);
          if (this.form.check_type == "order") {
            formData.append("start_date", this.form.dateRange.start_date);
            formData.append("end_date", this.form.dateRange.end_date);
          }

          let result = await this.$axios.post(
            "/advertisement/cheking-ads",
            formData,
            config
          );
          this.cancelSource = null;
