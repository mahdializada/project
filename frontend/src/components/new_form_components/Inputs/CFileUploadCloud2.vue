        ...
        <template slot="prepend-inner">
          ...
          >
            ...
            <p class="ma-0 prepend-text mb-1">
              Drag and Drop {{ dropFileType }} to upload
            </p>
      ...
      <template>
        <div class="items d-flex flex-wrap">
          <CFileItemsImage
            ...
            :key="i + 'index'"
            :file="file"
            @removeFile="removedUploadedFiles(index, file)"
          />
...
import CFileItemsCloud from "./CFileItemsCloud.vue";
import CFileItemsImage from "./CFileItemsImage.vue";
...
export default {
  ...
  watch: {},
  created() {
    this.attachments = this.$_.cloneDeep(this.prev_file);
  },
  methods: {
    onChange(files) {
      console.log("files", this.form);
      let temp = [];
      ...
      try {
        ...
        let config = {
          ...
          onUploadProgress: function (progressEvent) {
            ...
            this.updateProgress(item.id, uploadProgress);
          }.bind(this),
        };
        ...
        if (response.status == 200) {
          this.updateProgress(item.id, 100, response.data.path);
        } else {
    ...
    addtoRetry(item) {
      this.files = this.files.map((item2) => {
        if (item.id == item2.id) {
          item.retry = true;
        }
        return item2;
    ...
    cancelUpload(passedItem) {
      let item = this.files.find((item) => item.id == passedItem.id);
      if (item && item.source) {
    ...
    removeFile(file) {
      ...
      this.files = this.files.filter((image) => image.id != file.id);
    ...
    removedUploadedFiles(index, file) {
