<template>
  <div class="w-full h-full d-flex align-center justify-center">
    <div class="w-full h-full">
      <InputCard :title="$tr('upload_image')" v-bind="$attrs" v-on="$listeners">
        <template slot-scope="{ attrs, listeners }">
          <v-file-input
            ...
            v-on="listeners"
            @change="onChange"
            :rules="validateRule(form.images, $root, $tr('images'))"
            @blur="form.images.$touch()"
            :multiple="true"
          >
            <template slot="prepend-inner">
              ...
              >
                ...
                <p class="ma-0 prepend-text mb-1">
                  {{ $tr("drag_and_drop_images_to_upload") }}
                </p>
                <p class="ma-0 prepend-text2">
                  {{ $tr("or_browse_files_on_your_computer") }}
                </p>
              </div>
            </template>
          </v-file-input>
          <div class="items d-flex flex-wrap">
            <CFileItemsImage
              v-for="(file, i) in form.images.$model"
              :key="i"
              :file="file"
...
export default {
  ...
  methods: {
    validate() {
      ...
      let uploaded = true;
      this.form.images.$model.map((item) => {
        if (item.progress < 100) uploaded = false;
      });
      return !this.form.images.$invalid && uploaded;
    },
    ...
    onChange(files) {
      ...
      for (let i = 0; i < files?.length; i++) {
        ...
        temp.push({
          ...
          progress: 0,
          retry: false,
          preview: this.getImage(file),
        });
      }
      this.form.images.$model = [...this.form.images.$model, ...temp];
      temp.forEach((image) => {
        this.upload(image);
      });
    },
    ...
    async upload(item) {
      try {
        ...
        const request = new Promise(async function (resolve, reject) {
          ...
          item.source = source;
          const url = `product-management/pr-products/images`;
          app.$axios
            .post(url, formData, {
              ...
              onUploadProgress: function (progressEvent) {
                ...
                app.updateProgress(item.id, uploadProgress);
              }.bind(app),
            })
            .then((response) => {
              if (response.data.result == true) {
                app.updateProgress(item.id, 100, response.data.path);
              }
            })
            .catch(({ response }) => {
              app.addtoRetry(item);
    ...
    updateProgress(id, progress, path = "") {
      this.form.images.$model = this.form.images.$model.map((item) => {
        if (item.id == id) {
    ...
    addtoRetry(item) {
      this.form.images.$model = this.form.images.$model.map((item2) => {
        if (item.id == item2.id) {
          item.retry = true;
        }
        return item2;
    ...
    cancelUpload(passedItem) {
      let item = this.form.images.$model.find(
        (item) => item.id == passedItem.id
      );
      if (item && item.source) {
        item.source.cancel("file-cancled");
        delete item.source;
        this.addtoRetry(item);
      }
    },
    ...
    removeFile(file) {
      ...
      this.form.images.$model = this.form.images.$model.filter(
        (image) => image.id != file.id
      );
      if (file.progress == 100) {
        this.deleteFileBackend(file);
      }
    },
    ...
    async deleteFileBackend(item) {
      try {
        let res = await this.$axios.post(
          "product-management/pr-products/images",
          {
            file_path: item.path,
          }
        );
