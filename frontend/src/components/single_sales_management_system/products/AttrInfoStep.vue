						...
						<template v-slot:default>
							<thead>
								<tr class="text-body-1">
									<th
										v-for="(item, index) in selectedAttr"
										:key="index"
								...
								<tr class="pb-1" v-for="(item, i) in modelText" :key="i">
									...
									>
										<CTextField
											...
											hide-details
											:type="
												it.name == 'quantity' || it.name == 'price'
													? 'number'
													: 'text'
											"
											class="pt-0 mt-1"
    ...
    AddAtrr(item) {
      if (this.selectedAttr.findIndex((el) => el.id == item.id) > -1) {
        ...
        for (let index = 0; index < this.modelText.length; index++) {
          this.modelText[index] = this.modelText[index].filter(
            (r) => r.id !== item.id
          );
        }
      } else {
        ...
        for (let index = 0; index < this.modelText.length; index++) {
          ...
          this.modelText[index].splice(
            this.modelText[index].length - 2,
            0,
            item
          );
        }
...
.table
  table
  tbody
  tr:not(.v-data-table__selected):not(.not-hover-tr__force):hover {
