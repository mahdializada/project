<template>
  <v-card class="rounded-0 pb-3" elevation="1" width="100%">
    <v-card-title class="px-2 pt-2 pb-0">
      <span class="dialog-title ps-1"> {{ $tr("tabs_&_columns") }}</span>
      <v-spacer />
      <svg
        @click="closeDialog()"
        width="40px"
    ...
    <v-container fluid class="pt-0 pb-0">
      <v-row class="px-2">
        ...
        <v-col cols="12" md="4" class="px-1">
          <v-card elevation="5" height="450px" class="py-1 ps-3">
            <div>
            ...
            <div class="me-1 mt-2 list-card" id="custom-scroll">
              <v-list-item
                dense
                class="custom-category-list me-2"
                color="primary"
                :class="category.id == selected_category_id ? 'selected' : ''"
              ...
              >
                <v-list-item-content>
                  <v-list-item-title
                    class="custom-card-title-3 text-capital"
                ...
                <v-list-item-action>
                  <div class="d-flex align-center">
                    <v-chip
                      ...
                      v-html="getCategoryCount(category.id).count"
                    ></v-chip>
                    <v-icon>
                      {{
        ...
        <v-col cols="12" md="4" class="px-1">
          ...
          >
            <p class="mb-0 custom-card-title-1 pt-1">{{ $tr("columns") }}</p>
            <span
              class="select-all-btn me-5"
              :class="$vuetify.rtl ? 'rtl' : ''"
              @click="selectAll()"
              v-html="
                getCategoryCount(selected_category_id).count !=
                getCategoryCount(selected_category_id).all
                  ? `${$tr('select_all')}`
                  : `${$tr('unselect_all')}`
              "
            ></span>
            ...
            <div class="me-1 mt-2 list-card" id="custom-scroll">
              <v-list-item
                dense
                class="me-2 custom-column-list2 selected px-0 align-center"
                color="primary"
              ...
              >
                <!-- <v-list-item-icon>
                  <v-icon v-text="'mdi-eye'" color="#777" class="me-1"></v-icon>
                </v-list-item-icon> -->
                <v-list-item-content>
                  <v-list-item-title
                    class="custom-card-title-3"
                    style="line-height: 25px; z-index: 33333"
                    v-html="$tr(header.text)"
                  />
                </v-list-item-content>
                ...
                <v-list-item-action>
                  <div class="d-flex align-center">
                    <CustomizeColumnSetting
                      :page_name="page_name"
                      :props_column="header"
                      @saveColumnUpdates="saveColumnUpdates($event)"
                    />

                    <v-tooltip top color="primary" max-width="220px">
        ...
        <v-col cols="12" md="4" class="px-1">
          <v-card elevation="5" height="450px" class="py-1 px-3">
            <p class="mb-2 custom-card-title-1 pt-1">
              {{ $tr("columns_arrangement") }}
            </p>
            <!-- mdi-pan-vertical -->

            <div
              class="mt-2 py-2 list-card bg-custom-gray rounded"
              style="height: "
              id="custom-scroll"
            >
              <draggable
                v-model="all_headers.selected_headers"
                group="people"
                @start="() => {}"
                @end="() => {}"
                animation="200"
              ...
              >
                <v-list-item
                  dense
                  class="px-1 mb-1 mx-2 selected-header-list"
                  v-for="(header, index) in all_headers.selected_headers"
                  :key="index"
                  :ripple="false"
                >
                  <v-list-item-content>
                    <div class="d-flex">
                      ...
                      <v-list-item-title
                        class="custom-card-title-3 pb-1"
                        v-html="$tr(header.text)"
                      />
                    </div>
                  </v-list-item-content>
      ...
      <div class="d-flex">
        <div style="width: 60%" class="d-flex align-center">
          <CSelect
            ...
            :items.sync="all_views"
            :placeholder="$tr('shared_views')"
            item-text="name"
            item-value="id"
            dense
            :rounded="false"
  ...
  methods: {
    ...
    async submit() {
      ...
      if (check.length > 2 && !data) {
        ...
        this.$toasterNA(
          "red",
          this.$tr("you_cant_have_more_than_three_setting")
        );
    ...
    saveChanges(type = "save") {
      if (type == "save") {
        ...
        this.all_headers = JSON.parse(JSON.stringify(this.page_headers));
        this.closeDialog();
      }
    },
    searchHeader() {
