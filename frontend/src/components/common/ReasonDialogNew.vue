    ...
    <v-card :color="$vuetify.theme.dark ? 'black' : '#f9fafd'">
      ...
      <v-card-actions class="d-flex justify-end">
        <v-btn
          ...
          >{{ $tr("cancel") }}</v-btn
        >
        <v-btn class="primary" @click="onSubmit" v-if="isSubmiting">{{
          $tr("submit")
        }}</v-btn>
        <v-btn class="primary" @click="DoneReasonDialog" v-if="done">{{
          $tr("done")
        }}</v-btn>
      </v-card-actions>
    </v-card>
  ...
  methods: {
    async onSubmit() {
      ...
      if (this.selectedReasons.length > 0) {
        this.submiting = true;
        const result = await this.submitFunction(this.selectedReasons);
        if (result) {
    ...
    async fetchAllReasons(status) {
      try {
        ...
        const response = await this.$axios.get(
          `reasons/id?type=${status}&sub_system=${this.sub_system}`
        );
    ...
    successAnimation() {
      ...
      (this.isSubmiting = false), (this.done = true);
      setTimeout(() => {
        this.submiting = false;
        this.showReasonDialog = false;
        this.successMessage = false;
