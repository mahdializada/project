<template>
  <div class="countryFilter">
    <CustomFilter
      ...
      @onSubmit="onSubmit"
      @onClear="onClear"
      :cardTitle="$tr('filter_item', $tr('languages'))"
    >
      <template v-slot:data>
        <FilterInput
          v-model="form.name"
          :label="$tr('item_name', $tr('language'))"
        />
        <FilterInput v-model="form.native" :label="$tr('native')" />
      </template>
...
export default {
  ...
  methods: {
    ...
    sortData() {
      this.sortedData = {};
      if (this.form.name) this.sortedData.name = ['whereHasTwo', 'countryLanguage', 'language', this.form.name];
      
      if (this.form.native)
        this.sortedData.native = ['whereHasTwo', 'countryLanguage', 'language', this.form.native];
