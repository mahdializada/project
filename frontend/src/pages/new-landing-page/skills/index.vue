  ...
  <v-container fluid class="pa-0">
    <client-only>
      <v-dialog v-model="createFlag" persistent width="1300">
        <CreateSkill @closeDialog="createFlag = false" />
      </v-dialog>
      ...
      <v-row no-gutters>
        ...
        <v-col cols="12">
          <CustomPageActions
            ...
            :routeName="'landing/skills'"
            :subSystemName="'skills'"
...
import CreateSkill from "~/components/new_landing/skills/CreateSkill.vue";
import SkillCategory from "~/configs/pages/landing/skills/SkillCategory";
...
export default {
  ...
  data() {
    return {
      ...
      headers: SkillCategory(this).headers,
      tabItems: SkillCategory(this).tabItems,
      breadcrumb: SkillCategory(this).breadcrumb,
      selectedItems: [],
      tableKey: 0,
      tabIndex: 0,
      options: {},
  ...
  methods: {
    ...mapActions({
      fetchSkills: "new_landing/fetchItems",
    ...
    async fetchItems() {
      ...
      this.fetchSkills({ data: data, url: "landing/skills" });
    },
