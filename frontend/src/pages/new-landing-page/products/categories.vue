    ...
    <client-only>
      <v-dialog v-model="showRegisterDialog" persistent width="1300">
        <RegisterCategory
          ...
          api_url="landing/all-product-categories"
          store_url="landing/product-categories"
        />
      </v-dialog>
      ...
      <v-row no-gutters>
        ...
        <v-col cols="12">
          <CustomPageActions
            ...
            :routeName="'landing/product-categories'"
            :subSystemName="'Product Category'"
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumb: ProductCategories(this).CategoryBreadcrumb,
      selectedItems: [],
      tableKey: 0,
      tabIndex: 0,
      options: {},
      ...
      url: "landing/product-categories",
  ...
  methods: {
    ...
    async viewSubCategories(id) {
      window.location.pathname =
        "/new-landing-page/products/subcategories/" + id;
    },

    async fetchCategories(url) {
      let data = this.fetchOptions();
      this.fetchCategoriesData({ data, url });
    },
