...
export default {
			...
			.listen("PersonalFileChange", async (e) => {
				switch (e.action) {
					case "shared": {
						...
						if (item) {
							file.addItemToRoot(this.history, item.shareable);
						}
						break;
					}
					case "rename-item": {
	...
	methods: {
		...
		async onMySharedSearch(searchTerm) {
			if (searchTerm) {
				if (this.isSearching) return;
				try {
					this.isSearching = true;
					const { slug } = this.$route.params;
					let url = `files/personal/files/search?searchTerm=${searchTerm}&type=my_shared`;
					if (slug) {
						url += `&parent_id=${slug}`;
					}
		...
		async fetchParentItems(layoutGrid = true) {
			try {
				this.fetchingItems = true;
				const response = await this.$axios.get("files/personal/my_shares");
				const items = response.data.files;
