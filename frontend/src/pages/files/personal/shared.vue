<template>
	<div class="files-container background">
		<PersonalTopTitle
			:showNewButton="
				this.history.current_page.parent
					? this.history.current_page.parent.permission == 'read_&_write'
						? true
						: false
					: false
			"
			homeName="shared"
			:lastItem="getBreadcrumbItem(-1)"
			:prevItem="getBreadcrumbItem(-2)"
			:currentSortItem="history.current_page.sortBy"
			@onSearch="onSharedSearch"
			:searchLoading="isSearching"
			@submitFilter="submitFilter"
			@clearFilter="clearFilter"
		/>
		<NuxtChild
			ref="slugNuxtChild"
			:key="$route.fullPath"
			:currentPage="history.current_page"
			:loading="fetchingItems"
			:not_found="not_found"
		/>
	</div>
</template>

<script>
import PersonalTopTitle from "~/components/files/personal/PersonalTopTitle.vue";
import { mapMutations, mapState } from "vuex";
import FileManagement from "~/utils/file_management/FileManagement";
import moment from "moment-timezone";

export default {
	components: {
		PersonalTopTitle,
	},

	mounted() {
		this.$root.$on("onSortChanged", this.onSortChanged);
		this.$root.$on("onRefreshPage", this.refreshSharedItems);

		this.changeLayout("grid");
		if (process.client) {
			const { slug } = this.$route.params;
			if (slug) {
				this.fetchChildItems(slug);
			} else {
				this.fetchParentItems();
			}
		}
		// realtime
		const file = new FileManagement(this);
		this.$echo
			.private(`personal_file.${this.$auth.user.id}`)
			.listen("PersonalFileChange", async (e) => {
				switch (e.action) {
					case "shared": {
						let item = await file.getSharedFileRealTime(e?.data?.item_id);
						if (item) {
							file.addItemToRoot(this.history, item.shareable, item.seen);
						}
						break;
					}
					case "unshare-item": {
						file.deleteItemHistory(this.history, e.data, e.data.type + "s");
						break;
					}
					case "rename-item": {
						file.renameItemHistory(this.history, e.data, e.data.type + "s");
						break;
					}
					case "create-item": {
						let item = await file.getFileRealTime(e?.data?.item_id);
						if (item) {
							this.addFilesOrFolders(item, e?.data?.type == "file");
						}
						break;
					}
					case "delete-item": {
						file.deleteItemHistory(this.history, e.data, e.data.type + "s");
						break;
					}
					case "move-item": {
						file.deleteItemHistory(
							this.history,
							e.data,
							e.data.type + "s",
							true
						);
						let item = await file.getFileRealTime(e?.data?.item_id);
						if (item) {
							this.addFilesOrFolders(item, e?.data?.type == "file");
						}
						break;
					}
					case "update-item": {
						file.updateParent(this.history, e.data);
						break;
					}
					default:
						break;
				}
			});
	},

	beforeDestroy() {
		this.$nuxt.$off();
		this.$echo.leave(`personal_file.${this.$auth.user.id}`);
	},

	data() {
		return {
			fetchingItems: false,
			isSearching: false,
			history: {
				current_page: {
					sortBy: "name",
					unFilterdFiles: [],
					unFilterdFolders: [],
					files: [],
					folders: [],
					breadcrumb: [
						{
							to: "/files/personal/shared",
							exact: false,
							text: "shared",
							parent: true,
						},
					],
				},
			},
			showNewButton: false,
			not_found: false,
		};
	},

	computed: {
		...mapState("files", [
			"uploadedFiles",
			"uploadedFolders",
			"newFolderUploaded",
		]),
	},

	watch: {
		"$route.params.slug"() {
			this.onRouteChange();
		},
		uploadedFiles(value) {
			if (value[value.length - 1]) {
				let {
					response: { data },
				} = value[value.length - 1];
				this.addFilesOrFolders(data, true);
			}
		},
		newFolderUploaded(value) {
			let folders = this.uploadedFolders[value];
			// let { current_page, parent } = this.history;
			Object.values(folders).forEach((item) => {
				this.addFilesOrFolders(item, false);
			});
		},
	},

	methods: {
		...mapMutations("files", ["changeLayout"]),

		clearFilter() {
			this.resetFilterData();
		},

		resetFilterData() {
			this.history.current_page.files =
				this.history.current_page.unFilterdFiles;
			this.history.current_page.folders =
				this.history.current_page.unFilterdFolders;
		},

		submitFilter(filterData) {
			this.resetFilterData();

			if (filterData.selectedfileTypesId == "any") {
				if (filterData.selecteddateModifiedsId == "any_time") {
					this.resetFilterData();
				} else {
					this.swtichFilters(filterData);
				}
			} else if (filterData.selectedfileTypesId == "folder") {
				if (filterData.selecteddateModifiedsId == "any_time") {
					this.resetFilterData();
				} else {
					this.swtichFilters(filterData);
				}
				this.history.current_page.files = [];
			} else if (
				filterData.selectedfileTypesId == "document" &&
				filterData.selectedfileTypesDataTypeId == "any"
			) {
				this.history.current_page.folders = [];

				this.swtichFilters(filterData);
				this.history.current_page.files = filterData.selectedfileTypesId
					? this.history.current_page.files.filter((row) => {
							return (
								row.mime_type.split("/")[0] === "application" ||
								row.mime_type.split("/")[0] === "text"
							);
					  })
					: this.history.current_page.files;
			}
			// files filters
			else {
				this.filesFilter(filterData);
			}
		},

		filesFilter(filterData) {
			this.history.current_page.folders = [];
			if (filterData.selectedfileTypesDataTypeId == "any") {
				this.swtichFilters(filterData);
				this.history.current_page.files = filterData.selectedfileTypesId
					? this.history.current_page.files.filter(
							(row) =>
								row.mime_type.split("/")[0] === filterData.selectedfileTypesId
					  )
					: this.history.current_page.files;
			} else {
				this.swtichFilters(filterData);
				this.history.current_page.files = filterData.selectedfileTypesDataTypeId
					? this.history.current_page.files.filter(
							(row) => row.extension === filterData.selectedfileTypesDataTypeId
					  )
					: this.history.current_page.files;
			}
		},

		swtichFilters(val) {
			switch (val.selecteddateModifiedsId) {
				case "custom":
					if (val.enddate == "") {
						this.filterByDate(val.startdate);
					} else {
						this.history.current_page.files =
							this.history.current_page.files.filter((row) => {
								return (
									this.localeHumanReadableTime(row.created_at) >=
										this.localeHumanReadableTime(val.startdate) &&
									this.localeHumanReadableTime(row.created_at) <=
										this.localeHumanReadableTime(val.enddate)
								);
							});

						this.history.current_page.folders =
							this.history.current_page.folders.filter((row) => {
								return (
									this.localeHumanReadableTime(row.created_at) >=
										this.localeHumanReadableTime(val.startdate) &&
									this.localeHumanReadableTime(row.created_at) <=
										this.localeHumanReadableTime(val.enddate)
								);
							});
					}
					break;
				case "yesterday":
					var date = new Date();
					date.setDate(date.getDate() - 1);

					this.filterByDate(date);

					break;
				case "today":
					let today = new Date().toLocaleDateString();
					this.filterByDate(today);

					break;
				case "last_seven_day":
					var date = new Date();
					date.setDate(date.getDate() - 7);

					this.history.current_page.files =
						this.history.current_page.files.filter(
							(row) =>
								this.localeHumanReadableTime(row.created_at) >=
								this.localeHumanReadableTime(date)
						);
					this.history.current_page.folders =
						this.history.current_page.folders.filter(
							(row) =>
								this.localeHumanReadableTime(row.created_at) >=
								this.localeHumanReadableTime(date)
						);

					break;
				default:
			}
		},

		filterByDate(date) {
			this.history.current_page.files = this.history.current_page.files.filter(
				(row) =>
					this.localeHumanReadableTime(row.created_at) ===
					this.localeHumanReadableTime(date)
			);
			this.history.current_page.folders =
				this.history.current_page.folders.filter(
					(row) =>
						this.localeHumanReadableTime(row.created_at) ===
						this.localeHumanReadableTime(date)
				);
		},

		localeHumanReadableTime(date) {
			return moment(date)
				.locale(this.$vuetify.lang.current)
				.format("MMMM Do YYYY");
		},

		async onSharedSearch(searchTerm) {
			if (searchTerm) {
				if (this.isSearching) return;
				try {
					this.isSearching = true;
					const { slug } = this.$route.params;
					let url = `files/personal/files/search?searchTerm=${searchTerm}&type=shared`;
					if (slug) {
						url += `&parent_id=${slug}`;
					}
					const { data } = await this.$axios.get(url);
					const items = data.items;
					const folders = [];
					const files = [];
					const unFilterdFiles = [];
					const unFilterdFolders = [];
					for (let index = 0; index < items.length; index++) {
						const item = items[index];
						if (item.type == "folder") {
							folders.push(item);
							unFilterdFolders.push(item);
						} else {
							files.push(item);
							unFilterdFiles.push(item);
						}
					}

					this.history.current_page = {
						...this.history.current_page,
						folders,
						files,
						unFilterdFiles,
						unFilterdFolders,
					};
				} catch (_) {
					this.history.current_page = {
						...this.history.current_page,
						folders: [],
						files: [],
						unFilterdFiles: [],
						unFilterdFolders: [],
					};
				}
				this.isSearching = false;
			} else {
				this.onRouteChange();
			}
		},

		onRouteChange() {
			const { slug } = this.$route.params;
			if (slug) {
				if (slug in this.history) {
					const prevPage = this.history[slug];
					this.history.current_page = prevPage;
				} else {
					this.fetchChildItems(slug);
				}
			} else {
				this.fetchingItems = true;
				let parentPage = this.history.parent;
				if (parentPage) {
					parentPage.breadcrumb = [
						{
							to: "/files/personal/shared",
							exact: false,
							text: "shared",
						},
					];

					this.history.current_page = parentPage;
					this.fetchingItems = false;
				} else {
					this.fetchParentItems();
				}
			}
		},

		getBreadcrumbItem(index) {
			const { current_page } = this.history;
			const { breadcrumb } = current_page;
			if (breadcrumb) {
				if (Array.isArray(breadcrumb)) {
					return breadcrumb.slice(index)[0];
				}
				return {};
			}
			return {};
		},

		onSortChanged({ title }) {
			if (this.history.current_page.sortBy == title) return;
			this.history.current_page.sortBy = title;
			const { folders, files } = this.history.current_page;
			let sortedFolders = [];
			let sortedFiles = [];
			if (title == "name") {
				sortedFolders = folders.sort(this.sortByName);
				sortedFiles = files.sort(this.sortByName);
			} else if (title == "size") {
				sortedFolders = folders.sort(this.sortFolderBySize);
				sortedFiles = files.sort(this.sortFileBySize);
			} else if (title == "date_modified") {
				sortedFolders = folders.sort(this.sortFolderByDateModified);
				sortedFiles = files.sort(this.sortFolderByDateModified);
			}
			this.history.current_page.folders = sortedFolders;
			this.history.current_page.files = sortedFiles;
		},

		sortByName(itemA, itemB) {
			const nameA = itemA.name.toLowerCase();
			const nameB = itemB.name.toLowerCase();
			return nameA.localeCompare(nameB);
		},

		sortFileBySize(itemA, itemB) {
			let sizeA = itemA.size;
			let sizeB = itemB.size;
			if (sizeA < sizeB) return 1;
			if (sizeA > sizeB) return -1;
			return 0;
		},

		sortFolderBySize(itemA, itemB) {
			let sizeA = itemA.info.size;
			let sizeB = itemB.info.size;
			if (sizeA < sizeB) return 1;
			if (sizeA > sizeB) return -1;
			return 0;
		},

		sortFolderByDateModified(itemA, itemB) {
			let dateA = new Date(itemA.created_at);
			let dateB = new Date(itemB.created_at);
			if (dateA < dateB) return -1;
			if (dateA > dateB) return 1;
			return 0;
		},

		async fetchChildItems(slug) {
			try {
				this.fetchingItems = true;
				const response = await this.$axios.get(`files/personal/shared/${slug}`);
				if (response.data.not_found) {
					this.not_found = true;
					this.history.current_page = {
						...this.history.current_page,
						folders: [],
						files: [],
						unFilterdFiles: [],
						unFilterdFolders: [],
					};
					this.fetchingItems = false;
					return;
				}
				this.not_found = false;
				const items = response.data.files;
				const breadcrumb = response.data.breadcrumb;
				const parent = response.data.parent;
				const permission = response.data.permission;
				const sort = response.data.sort;
				const folders = [];
				const files = [];
				const unFilterdFiles = [];
				const unFilterdFolders = [];
				for (let index = 0; index < items.length; index++) {
					const item = items[index];
					if (item?.type == "folder") {
						folders.push({
							...item,
							isSelected: false,
							permission,
						});

						unFilterdFolders.push({
							...item,
							isSelected: false,
							permission,
						});
					} else {
						files.push({
							...item,
							isSelected: false,
							permission,
						});
						unFilterdFiles.push({
							...item,
							isSelected: false,
							permission,
						});
					}
				}

				this.history.current_page = {
					...this.history.current_page,
					folders,
					files,
					unFilterdFiles,
					unFilterdFolders,
					breadcrumb,
					parent: { ...parent, permission },
					sortBy: "",
				};
				this.onSortChanged({ title: sort });
				this.history.current_page.sortBy = sort;
				let currentPage = this.history.current_page;
				this.history[slug] = currentPage;
			} catch (_) {
				this.history.current_page = {
					...this.history.current_page,
					folders: [],
					files: [],
					unFilterdFiles: [],
					unFilterdFolders: [],
					breadcrumb: [
						{
							to: "/files/personal/shared",
							exact: false,
							text: "shared",
						},
					],
				};
			}
			this.fetchingItems = false;
		},

		async fetchParentItems(layoutGrid = true) {
			try {
				this.fetchingItems = true;
				const response = await this.$axios.get("files/personal/shared");
				const items = response.data.files;
				const layout = response.data.layout;
				if (layoutGrid) {
					this.changeLayout(layout);
				}
				const sort = response.data.sort;
				const folders = [];
				const files = [];

				const unFilterdFiles = [];
				const unFilterdFolders = [];

				for (let index = 0; index < items.length; index++) {
					const item = items[index];
					if (item?.shareable) {
						if (item?.shareable?.type == "folder") {
							folders.push({
								...item.shareable,
								isSelected: false,
								seen: item.seen,
								permission: item.permission,
							});
							unFilterdFolders.push({
								...item.shareable,
								isSelected: false,
								seen: item.seen,
								permission: item.permission,
							});
						} else {
							files.push({
								...item.shareable,
								isSelected: false,
								seen: item.seen,
								permission: item.permission,
							});

							unFilterdFiles.push({
								...item.shareable,
								isSelected: false,
								seen: item.seen,
								permission: item.permission,
							});
						}
					}
				}

				this.history.current_page = {
					...this.history.current_page,
					sortBy: "",
					folders,
					files,
					unFilterdFiles,
					unFilterdFolders,
					parent: null,
					breadcrumb: [
						{
							to: "/files/personal/shared",
							exact: false,
							text: "shared",
							parent: true,
						},
					],
				};
				this.onSortChanged({ title: sort });
				this.history.current_page.sortBy = sort;
				this.history.parent = this.history.current_page;
			} catch (_) {}
			this.fetchingItems = false;
		},

		isInArr(arr, passedItem) {
			let res = arr.find((item) => passedItem.id == item.id);
			return res !== undefined;
		},

		addFilesOrFolders(data, isFile) {
			data = JSON.parse(JSON.stringify(data));
			let fileOrFolder = isFile ? "files" : "folders";
			let { current_page, parent } = this.history;
			if (data.parent_id == null && current_page.parent == null) {
				if (!this.isInArr(current_page[fileOrFolder], data)) {
					current_page[fileOrFolder].unshift(data);
				}
			} else if (data.parent_id == null && current_page.parent !== null) {
				// for history
				if (parent && !this.isInArr(parent[fileOrFolder], data)) {
					parent[fileOrFolder].unshift(data);
				}
			} else if (
				data.parent_id !== null &&
				current_page.parent !== null &&
				data.parent_id == current_page.parent.id
			) {
				if (!this.isInArr(current_page[fileOrFolder], data)) {
					current_page[fileOrFolder].unshift(data);
				}
			} else if (
				current_page.parent !== null &&
				data.parent_id !== current_page.parent.id
			) {
				// for history
				if (data.parent_id == null) {
					if (parent && !this.isInArr(parent[fileOrFolder], data)) {
						parent[fileOrFolder].unshift(data);
					}
				} else {
					if (this.history[data.parent_id]) {
						if (
							!this.isInArr(this.history[data.parent_id][fileOrFolder], data)
						) {
							this.history[data.parent_id][fileOrFolder].unshift(data);
						}
					}
				}
			}
		},

		refreshSharedItems() {
			const { slug } = this.$route.params;
			if (slug) {
				this.fetchChildItems(slug);
			} else {
				this.fetchParentItems(false);
			}
			this.history = {
				current_page: { ...this.history.current_page },
			};
			if (this.$refs.slugNuxtChild) {
				this.$refs.slugNuxtChild.selectedCount = 0;
			}
		},
	},
};
</script>
