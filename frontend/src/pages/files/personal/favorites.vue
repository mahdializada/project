    ...
    pushNewItemsIntoParent(items) {
      ...
      if (Array.isArray(items)) {
		...
		async onSearchPersonalFiles(searchTerm) {
			if (searchTerm) {
				...
				try {
					...
					const { slug } = this.$route.params;
					let url = `files/personal/files/search?searchTerm=${searchTerm}&type=favorites`;
					if (slug) {
						url += `&parent_id=${slug}`;
					}
		...
		async fetchParentItems(layoutGrid = true) {
			try {
				this.fetchingItems = true;
				const response = await this.$axios.get("files/personal/favorites");
				const items = response.data.files;
