<template>
  <div class="files-container background">
    <PersonalTopTitle
      :lastItem="getBreadcrumbItem(-1)"
      :prevItem="getBreadcrumbItem(-2)"
      :currentSortItem="history.current_page.sortBy"
      @onSearch="onSearchPersonalFiles"
...
export default {
  ...
  beforeDestroy() {
    ...
    this.$echo.leave(`personal_file.${this.$auth.user.id}`);
  },

  mounted() {
    this.$root.$on("onSortChanged", this.onSortChanged);
    this.$root.$on("onRefreshPage", this.refreshDriveItems);
    this.toggleCreateFolder(false);
    ...
    // realtime
    const file = new FileManagement(this);
    this.$echo.leave(`personal_file.${this.$auth.user.id}`);
    this.$echo
      .private(`personal_file.${this.$auth.user.id}`)
      .listen("PersonalFileChange", async (e) => {
        switch (e.action) {
          case "rename-item": {
            file.renameItemHistory(this.history, e.data, e.data.type + "s");
            break;
          }
          case "create-item": {
            let item = await file.getFileRealTime(e?.data?.item_id);
            if (item) {
              this.addFilesOrFolders(item, e?.data?.type == "file");
            }
            break;
          }
          case "delete-item": {
            file.deleteItemHistory(this.history, e.data, e.data.type + "s");
            break;
          }
          case "update-item": {
            file.updateParent(this.history, e.data);
            break;
          }
          default:
            break;
  ...
  methods: {
    ...mapMutations("files", ["changeLayout", "toggleCreateFolder"]),

    onCopySuccess({ action, actualPrevItems, items }) {
      try {
        this.pushNewItemsIntoParent(items);
        if (action == "move") {
          let { files, folders } = this.history.current_page;
          actualPrevItems.forEach((item) => {
            if (item.type == "folder") {
              folders = folders.filter(({ id }) => id != item.id);
            } else {
              files = files.filter(({ id }) => id != item.id);
            }
          });
          this.history.current_page.files = files;
          this.history.current_page.folders = folders;
        }
      } catch (error) {}
    },

    pushNewItemsIntoParent(items) {
      const isNotFound = (items, item) => {
        let index = items.findIndex((arrayItem) => arrayItem.id == item.id);
        return index == -1;
      };
      if (Array.isArray(items)) {
        const eachItem = (item) => {
          let { parent_id } = item;
          if (parent_id && parent_id in this.history) {
            if (item.type == "folder") {
              if (isNotFound(this.history[parent_id].folders, item)) {
                this.history[parent_id].folders.push(item);
              }
            } else {
              if (isNotFound(this.history[parent_id].files, item)) {
                this.history[parent_id].files.push(item);
              }
            }
            let { parent } = this.history.current_page;
            if (parent && parent_id == parent.id) {
              if (item.type == "folder") {
                if (isNotFound(this.history.current_page.folders, item)) {
                  this.history.current_page.folders.push(item);
                }
              } else {
                if (isNotFound(this.history.current_page.files, item)) {
                  this.history.current_page.files.push(item);
                }
              }
            }
          }
          if (this.history.parent && parent_id == null) {
            if (item.type == "folder") {
              if (isNotFound(this.history.parent.folders, item)) {
                this.history.parent.folders.push(item);
              }
            } else {
              if (isNotFound(this.history.parent.files, item)) {
                this.history.parent.files.push(item);
              }
            }
          }
        };
    ...
    refreshDriveItems() {
      ...
      if (slug) {
        ...
        this.fetchParentItems(false);
      }
      this.history = {
        current_page: { ...this.history.current_page },
      };
      if (this.$refs.slugNuxtChild) {
        this.$refs.slugNuxtChild.selectedCount = 0;
      }
    },
    ...
    async fetchChildItems(slug) {
      try {
        ...
        this.onSortChanged({ title: sort });
        this.history.current_page.sortBy = sort;
    ...
    async fetchParentItems(layoutGrid = true) {
      try {
        this.fetchingItems = true;
        const response = await this.$axios.get("files/personal");
        const items = response.data.files;
    ...
    isInArr(arr, passedItem) {
      let res = arr.find((item) => passedItem.id == item.id);
    ...
    addFilesOrFolders(data, isFile) {
      ...
      if (data.parent_id == null && current_page.parent == null) {
        if (!this.isInArr(current_page[fileOrFolder], data)) {
          current_page[fileOrFolder].unshift(data);
        }
      } else if (data.parent_id == null && current_page.parent !== null) {
        // for history
        if (parent && !this.isInArr(parent[fileOrFolder], data)) {
        ...
        if (data.parent_id == null) {
          ...
          if (this.history[data.parent_id]) {
            if (
              !this.isInArr(this.history[data.parent_id][fileOrFolder], data)
            ) {
              this.history[data.parent_id][fileOrFolder].unshift(data);
            }
          }
        }
      }
