...
import FileFilters from "~/components/files/common/FileFilters.vue";
...
export default {
  ...
  methods: {
    ...
    async fetchChildItems(slug) {
      try {
        this.fetchingItems = true;
        const response = await this.$axios.get(`files/personal/trash/${slug}`);
    ...
    async fetchParentItems(layoutGrid = true) {
      try {
        this.fetchingItems = true;
        const response = await this.$axios.get("files/personal/trash");
        const items = response.data.files;
    ...
    async onSearchTrashFiles(searchTerm) {
      if (searchTerm) {
        if (this.isSearching) return;
        try {
          this.isSearching = true;
          const { slug } = this.$route.params;
          let url = `files/personal/files/search?searchTerm=${searchTerm}&type=trashed`;
          if (slug) {
            url += `&parent_id=${slug}`;
          }
