    ...
    >
      ...
      <div v-else class="h-full">
        ...
        >
          ...
          <div class="d-flex flex-wrap px-2">
            ...
            <Folder
              ...
              :folder="folder"
              :index="folderIndex"
              :link="`/files/personal/favorites/${folder.id}`"
              actionUrl="files/personal/folder/action"
              :parent_id="currentPage.parent && currentPage.parent.id"
              showFavorite
        ...
        <div v-else-if="not_found" style="height: 100%" class="content-section">
          ...
          >
            <svg width="80" viewBox="0 0 700 540" fill="rgba(0,0,0,0.15)">
              <path
                d="m349.92 105h210.08c38.66 0 70 31.34 70 70v280c0 38.66-31.34 70-70 70h-420c-38.66 0-70-31.34-70-70v-280c0-2.9961 0.1875-5.9492 0.55469-8.8477-0.36719-2.8594-0.55469-5.75-0.55469-8.6523v-52.5c0-38.66 31.34-70 70-70h140zm0.078125 249.09 70.395 70.395 35.199-35.199-70.395-70.395 70.395-70.395-35.199-35.199-70.395 70.395-70.395-70.395-35.199 35.199 70.395 70.395-70.395 70.395 35.199 35.199z"
                fill-rule="evenodd"
              />
            </svg>
  ...
  methods: {
    ...
    onPreviewClose(isInPipMode) {
      ...
      if (this.$route.query.preview) {
        this.$router.push("/files/personal/favorites?preview=false");
      }
    },
    ...
    async addToFavorites(items) {
      ...
      try {
        if (!items) {
          const { id, name, type, favorites, parent_id } = this.selectedItem;
          items = [{ id, name, type, favorites, parent_id }];
        }

        if (items.length > 0) {
          const firstItem = items[0];
          if (firstItem.favorites && firstItem.parent_id == null) {
            let { files, folders } = this.currentPage;
            let removeResult = fileManagement.removeItemsAndReturn({
              items,
              files,
    ...
    onItemRightClick({ event, item }) {
      ...
      // if (item.type != "folder") menuItems.splice(1, 1);
      const options = {
        items: menuItems,
        showMenu: true,
