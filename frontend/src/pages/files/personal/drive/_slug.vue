				...
				>
					...
					<div class="d-flex flex-wrap px-2">
						...
						<Folder
							...
							:folder="folder"
							:index="folderIndex"
							:link="`/files/personal/drive/${folder.id}`"
							actionUrl="files/personal/folder/action"
							:parent_id="currentPage.parent && currentPage.parent.id"
							showFavorite
...
export default {
	...
	methods: {
		...
		onPasswordCorrect({ item, currentPassword }) {
			try {
				this.$emit("nextItemPassword", item);
				if (item.type == "folder") {
					const { folders } = this.currentPage;
					const index = folders.findIndex(({ id }) => id == item.id);
					if (index != -1) {
						const { info } = item;
						const element = {
							...folders[index],
						...
						this.currentPage.folders.splice(index, 1, element);
					}
				} else {
					const { files } = this.currentPage;
					const index = files.findIndex(({ id }) => id == item.id);
					if (index != -1) {
						const { path, size, thumbnail } = item;
						const element = {
							...files[index],
							is_protected: false,
							size,
						...
						this.currentPage.files.splice(index, 1, element);
					}
				}
			} catch (error) {}
		},
		...
		onCustomShortKeys({ srcKey }) {
			...
			if (srcKey == "create_folder") {
				this.$root.$emit("onCreateMenuClicked", "create_folder");
				return;
			} else if (srcKey == "refresh_page") {
		...
		onPreviewClose(isInPipMode) {
			...
			if (this.$route.query.preview) {
				this.$router.push("/files/personal/drive?preview=false");
			}
		},
		...
		openFolder({ isNewWindow, item }) {
			...
			let { id, type, is_protected } = item;
			//   if (is_protected && !isNewWindow) {
			//     let action = {
			//       ...item,
			//       modelType: "open_item_model",
			//       passwordActionType: "open-folder",
			//     };
			//     this.$root.$emit("togglePasswordModel", action);
			//     return;
			//   }
		...
		dbClickHandler(item) {
			...
			//       ...item,
			//       modelType: "open_item_model",
			//       passwordActionType: "open-file-item",
			//     };
			//     this.$root.$emit("togglePasswordModel", action);
			//     return;
			//   }
