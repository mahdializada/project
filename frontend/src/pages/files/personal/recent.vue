...
export default {
  ...
  mounted() {
    ...
    this.$root.$on("onRefreshPage", this.refreshRecentItems);
  ...
  methods: {
    ...
    async onSearchPersonalFiles(searchTerm) {
      if (searchTerm) {
        ...
        try {
          ...
          const { slug } = this.$route.params;
          let url = `files/personal/files/search?searchTerm=${searchTerm}&type=recent`;
          if (slug) {
            url += `&parent_id=${slug}`;
          }
    ...
    async fetchChildItems(slug) {
      try {
        this.fetchingItems = true;
        const response = await this.$axios.get(`files/personal/recent/${slug}`);
    ...
    async fetchParentItems(layoutGrid = true) {
      try {
        this.fetchingItems = true;
        const response = await this.$axios.get("files/personal/recent");
        const items = response.data.files;
