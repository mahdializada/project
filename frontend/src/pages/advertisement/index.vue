<template>
  <v-container fluid class="pa-0">
    ...
    />
    <ChangeStatusItemCode
      @changeStatus="changeItemStatus"
      ref="ChangeStatusItemCodeRef"
    />
    <ShowProductProfileInfoImage ref="ShowProductProfileInfoImageRef" />
    <CMahdi ref="target" />
    <OtpCodeForm ref="otpCodeRefs" @onSuccess="deleteItemCode(true)" />
            ...
            <template slot="afterDelete">
              ...
              /> -->
                  ...
                  >
                    ...
                    >
                      <v-spacer></v-spacer>
                      <v-btn text color="primary" @click="crm_menu = false">
                        Cancel
                      </v-btn>
                      <v-btn text color="primary" @click="refreshCrm()">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-menu>
...
import ChangeStatusItemCode from "../../components/alert/ChangeStatusItemCode.vue";
...
export default {
  ...
  data() {
    return {
      crm_menu: false,
      crm_date: moment().format("YYYY-MM-DD"),
  ...
  methods: {
    ...
    async deleteAds() {
      ...
      if (this.deleteDate != null) {
        ...
        if (response.status == 200) {
          ...
          this.$toasterNA("green", this.$tr("ads_deleted_successfully"));
        } else {
          this.$toasterNA("error", this.$tr("something_went_wrong"));
        }
      } else {
        this.$toasterNA("error", this.$tr("select ad date please"));
      }
    },
    ...
    toggleItemStatus() {
      this.$refs.ChangeStatusItemCodeRef.toggleStatusDialog(
      ...
      try {
        ...
        if (result.status == 201) {
          setTimeout(() => {
            ...
            this.$refs.ChangeStatusItemCodeRef.close();
          }, 1000);
        } else {
          this.$refs.ChangeStatusItemCodeRef.reset();
    ...
    deleteItemCode(fromEmit = false) {
      ...
      this.$TalkhAlertNA(
        ...
        "delete", //icon
        async () => {
          this.$refs.otpCodeRefs.toggleDialog(this.currentTab.selectedItems[0]);
        }, // callback function
        "yes_delete" // btn text
      );
    },
