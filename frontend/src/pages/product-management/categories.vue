      ...
      <v-row no-gutters>
        ...
        <v-col cols="12">
          <CustomPageActions
            ...
            :routeName="'single-sales/categories'"
...
import CategoryStepper from "~/components/horizontal_stepper/CategorySteps/CategoryStepper.vue";
  ...
  methods: {
    ...
    async getRecord() {
      try {
        ...
        const response = await this.$axios.get("product-management/pr-categories", {
          params: data,

        });
        if (response.status === 200) {
          this.tableValues = response?.data;
          console.log("hhjhd",response.data);
