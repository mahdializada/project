<template>
  <v-container fluid class="pa-0">
    <AttribueteStepper ref="attributeInsertRef" @pushRecord="pushRecord" />
          ...
          >
            ...
            <template v-slot:[`item.values`]="{ item }">
              <v-tooltip bottom max-width="200">
                ...
                <span v-for="value in valueToJson(item.values)" :key="value">
                  {{ value }} <br />
                </span>
              </v-tooltip>
            </template>
  ...
  methods: {
    ...
    toggleEditDialog() {
      if (this.selectedItems.length == 1) {
        ...
        this.$refs.updateAttributeStepper.showDialog(parsedItems);
        this.editMode = true;
      } else {
