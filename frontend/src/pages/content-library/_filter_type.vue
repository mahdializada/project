...
export default {
  ...
  methods: {
    ...
    async getMedia(data = null) {
      ...
      try {
        ...
        if (response.status == 200) {
          ...
          this.extraData = [];
          var all = 0;
          response.data.extraData.map((item)=>{
            if(item.status == "publish")
            this.extraData['publish'] = item.count
            else if(item.status == 'not publish')
            this.extraData['not_publish'] = item.count
            else if(item.status=='rejected')
            this.extraData['rejected'] = item.count
            all+=item.count;
          });
          this.extraData['all'] = all;
          this.$root.$emit("extraDataInContentLibrary",this.extraData);
        }
      } catch (error) {
    ...
    addRecord(item) {
      this.getMedia(this.tabKey);
      // if (this.tabKey == "not publish" || this.tabKey == "all")
      //   for (let i = 0; i < item.length; i++) {
      //     this.medias.unshift(item[i]);
      //   }
    },
    expandOnScroll() {
