      ...
      <v-row no-gutters>
        ...
        <v-col cols="12">
          <CustomPageActions
            ...
            :routeName="'single-sales/brand-ssp'"
...
import BrandView from "../../components/single_sales_management_system/brand/BrandView.vue";
export default {
  components: {
    CustomPageActions,
    PageActions,
  ...
  methods: {
    ...
    toggleViewDialog (){
      ...
      if(this.selectedItems.length == 1){
        ...
        this.$refs.brandView.toggleViewDialog(item);
    ...
    async getRecord() {
      try {
        ...
        const response = await this.$axios.get("single-sales/brand-ssp", {
          params: data,
        });
    ...
    setStatusItems(status) {
      switch (status) {
        case "active":
          this.statusItems = [{ id: "inactive", name: "inactive" },{ id: "banned", name: "banned" }];
          break;
        case "inactive":
          this.statusItems = [{ id: "active", name: "active" },{ id: "banned", name: "banned" }];
          break;
        case "banned":
    ...
    getTotals(tabKey) {
      switch (tabKey) {
        ...
        case "banned":
          return this.extraData?.bannedTotal || 0;
      }
    },

    async pushRecord(data) {
      console.log(data, this.tableValues);
      if (this.tabKey === "all" || this.tabKey === "inactive")
      for (let index = 0; index < data.length; index++) {
        await this.tableValues.unshift(data[index]);
