<template>
  ...
  <v-container fluid class="pa-0">
    ...
    <!-- <productUpdateStepper
      ...
      @getRecord="getRecord"
    ></productUpdateStepper> -->
    <ProductProfileView2 ref="productProfileViewRef2" @updateFile="updateFile" />
    <ProductProfileView ref="productProfileViewRef" />
    <!-- <v-btn @click="toggleViewProfile()">view profile</v-btn> -->
      ...
      <v-row no-gutters>
        ...
        >
          <!-- <CustomButton
            v-if="$isInScope('advc')"
            icon="fa-plus"
            @click="openInventoryInsertion"
            :text="$tr('create_item', $tr('Inventory'))"
            type="warning"
          /> -->
          <CustomButton
            v-if="$isInScope('advc')"
        ...
        <v-col cols="12">
          ...
          >
            ...
            <template v-slot:[`item.inventory`]="{ item }">
							<span @click="viewInventory(item)" class="mx-1 font-weight-bold">
								<v-icon
									class="primary--text"
            ...
            <template v-slot:[`item.company`]="{ item }">
              <v-tooltip bottom max-width="200">
                ...
                <div v-for="(data,index) in item.company" :key="index" class="d-flex" style="width:190px">
                  <div style="width:20%">
                    <v-img
                      class="rounded-circle"
                      width="25"
            ...
            <template v-slot:[`item.attribute`]="{ item }">
              <v-tooltip bottom max-width="250">
               ...
               <span v-for="(data,index) in item.product_attributes" :key="index">
                {{ data?.attributes?.name }} <br/>
               </span>

              </v-tooltip>
            </template>
            <template v-slot:[`item.attribute_values`]="{ item }">
              <v-tooltip bottom max-width="200">
                <template v-slot:activator="{ on, attrs }">
                  <span
                    v-bind="attrs"
                ...
                <span v-for="(data,index) in item.product_attributes" :key="index">
                {{ data?.attribute_value }} <br/>
               </span>

              </v-tooltip>
            </template>
            <template v-slot:[`item.arabic_description`]="{ item }">
              <v-tooltip bottom max-width="200">
                <template v-slot:activator="{ on, attrs }">
                  <span
                    v-bind="attrs"
                  ...
                  >
                    <span class="mx-auto d-flex justify-center">
                      {{ item.arabic_description?.substring(0, 30) + " ..." }}
                    </span>
                  </span>
                </template>
...
import StepperComponent from "../../../components/single_sales_management_system/products/StepperComponent.vue";
import ProductInventory from "../../../components/single_sales_management_system/products/ProductInventory.vue";
import Dialog from "../../../components/design/Dialog/Dialog.vue";
import ProductProfileView from "../../../components/single_sales_management_system/products/ProductProfileView.vue";
import ProductProfileView2 from "../../../components/single_sales_management_system/products/ProductProfileView2.vue";
import ViewInventory from "../../../components/single_sales_management_system/products/ViewInventory.vue";
  ...
  methods: {
    updateFile(item){
      this.tableValues = this.tableValues.map((i)=>{
        if(i.id == item.id){
          i.attachments = item.attachments
    ...
    openInventoryInsertion() {
      if (this.selectedItems.length > 0) {
        if (this.selectedItems.length == 1) {
          // this.InventoryModel = true;
            this.$refs.refInventory?.openModelInventory();
        } else {
    ...
    toggleViewProfile() {
      ...
      if (this.selectedItems.length == 1) {
        const items = this.selectedItems[0];
        this.$refs.productProfileViewRef2.showViewDialog(items);
      } else {
        const items = this.selectedItems[0];
        this.$refs.productProfileViewRef.showViewDialog(items);
      }
    },
     // inventory view
     viewInventory(item){
      this.$refs.inventoryView.viewModel(item);
    },
    ...
    async getRecord() {
      try {
        ...
        const response = await this.$axios.get("single-sales/products-ssp", {
          params: data,
        });
