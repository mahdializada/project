<template>
  <v-container fluid class="pa-0">
    <client-only>
      <PlatformOperation
        v-if="$isInScope('advplatformsc') || $isInScope('advplatformsu')"
        ref="platformOperationRef"
      />
      <PlatformStepper ref="platformStepperRef" @pushRecord="pushRecord"/>
      <PlatformFilter ref="platformFilter" @filterRecords="onFilterRecords" />
      ...
      <v-row no-gutters>
        ...
        <v-col cols="12">
          <PageFilters
            ...
            :downloadContent="platformRecords"
            :downloadDialog="downloadDialog"
            :showDownload="$isInScope('advplatformse')"
          ...
          >
            <!-- <CustomButton
              v-if="$isInScope('advplatformsc')"
              icon="fa-plus"
              @click="openInsertDialog"
              :text="$tr('create_item', $tr('platform'))"
              type="primary"
            /> -->
            <CustomButton
              v-if="$isInScope('advplatformsc')"
              icon="fa-plus"
              @click="showDialog"
              :text="$tr('create_item', $tr('platforms'))"
              type="primary"
            />
          </PageFilters>
        </v-col>
        ...
        <v-col cols="12">
          <CustomPageActions
            :showView="false"
            :showEdit="$isInScope('advplatformsu')"
            :showAutoEdit="false"
            :showDelete="false"
            :ignoreReason="true"
            :showSelect="false"
...
import Platform from "~/configs/pages/advertisement/platform";
...
import PlatformOperation from "../../components/advertisement/PlatformOperation.vue";
import Alert from "../../helpers/alert";
import PlatformFilter from "../../components/advertisement/PlatformFilter.vue";
import PlatformStepper from "../../components/advertisement/platforms/platformStepper.vue";
...
export default {
  ...
  data() {
    return {
      ...
      tabItems: Platform(this).tabItems,
      breadcrumb: Platform(this).breadcrumb,
      selectedItems: [],
      showProgress: false,
      tableKey: 0,
      tabIndex: 0,
  ...
  methods: {
    ...
    async onRestore() {
      if (this.getTabKey() == "removed") {
        try {
          ...
          const url = `advertisement/platforms/update?restore=true`;
    ...
    async onDelete() {
      ...
      try {
        const ids = this.selectedItems.map(({ id }) => id);
        const url = `advertisement/platforms/${ids}`;
...
;
    ...
    openInsertDialog() {
      this.$refs.platformOperationRef.toggle();
    },
    showDialog() {
      this.$refs.platformStepperRef.showDialog();
    },

    openFilterDialog() {
      this.$refs.platformFilter.changeModel();
    },
    ...
    async fetchDataAccordingtoStatus() {
      ...
      const response = await this.$axios.get(`advertisement/platforms`, {
        params: data,
      });
      if (response?.status === 200) {
        this.platformRecords = response.data?.data;
    ...
    async searchById(id) {
      ...
      try {
        ...
        const response = await this.$axios.get(
          "advertisement/platforms/search",
          {
            params: options,
          }
        );
    ...
    selectData(response) {
      if (response.status === 200) {
        ...
        if (data != null) {
          this.platformRecords = this.platformRecords.filter(
            (item) => item.id !== data.id
          );
          this.platformRecords?.unshift(data);
    ...
    async onEdit() {
      if (this.selectedItems.length == 1) {
        ...
        const { platform_key, platform_name, id, companies } = firstItem;
        const item = {
          platform_key,
          platform_name,
          id,
        ...
        // this.$refs.platformOperationRef.toggle(firstItem);
        this.$refs.platformStepperRef.showDialog(firstItem);
      } else {
