            ...
            <template slot="afterDelete">
              <div class="d-flex">
                ...
                >
                  ...
                  <CustomButton
                    ...
                    text="Bank Card"
                    type="primary"
                    v-if="currentTab.key == 'ad_account'&& currentTab.selectedItems && currentTab.selectedItems.length == 1"
                  ...
                  />
                  <CustomButton
                    v-if="currentTab.key == 'item_code' && !currentTab?.selectedItems[0]?.product_profile_info?.id &&
              ...
              /> -->
                  ...
                  >
                    <template v-slot:activator>
                      <CustomButton
                        ...
                        v-if=" currentTab.selectedItems && currentTab.selectedItems.length == 1"
                        icon="fa-tag"
                        text="label"
                        type="label"
                      />
                  ...
                  <CustomButton
                    ...
                    v-if="currentTab.key=='item_code' && currentTab.selectedItems && currentTab.selectedItems.length == 1"
                    icon="fa-toggle-off"
                    text="Item Status"
                    type="refresh"
                  />
                  ...
                  >
                    ...
                    >
                      <v-spacer></v-spacer>
                      <v-btn text color="primary" @click="deleteMenu = false">
                        Cancel
                      </v-btn>
                      <v-btn text color="primary" @click="deleteAds()"  :loading="delete_loading">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-menu>
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <div v-bind="attrs" v-on="on" class="">
                        <RefreshPercentage
                          v-if="date_rf_percentage > 0 || showMissedAds"
                          :percentage="date_rf_percentage"
                          type="refresh"
                          :textColor="refresh_percentage_color"
                          :position="true"
                    ...
                    >
                      ...
                      <v-btn text color="primary" @click="refreshMissedAds()">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-menu>
        ...
        <v-col cols="12">
          ...
          >
            ...
            <template v-slot:[`item.total_ad_account_balance`]="{ item }">
              <span>
                {{ item.total_ad_account_balance%1!=0?item.total_ad_account_balance?.toFixed(2):item.total_ad_account_balance }}
              </span>
            </template>
            ...
            <template v-slot:[`item.total_balance`]="{ item }">
              {{ item.ad_account_balance%1!=0?item.ad_account_balance?.toFixed(2):item.ad_account_balance }}
            </template>
...
import ItemCodeDelationForm from "../../components/advertisement/ItemCodeDelationForm.vue";
...
export default {
  ...
  methods: {
    async deleteAds() {
      ...
      if (this.deleteDate != null) {
        ...
        if (response.status == 200) {
          ...
          this.$toasterNA(
            "green",
            this.$tr("ads_deleted_successfully")
          );
        } else {
          this.$toasterNA(
            "error",
            this.$tr("something_went_wrong")
          );
     }
      } else {
        this.$toasterNA(
            "error",
            this.$tr("select ad date please")
          );
      }
    },
    ...
    async onItemCodeTabChange(tab) {
      ...
      if(tab !='all')
       this.all_headers[4].selected_headers = this.all_headers[4].selected_headers.filter((item)=>item.value!='item_status');
      else{
        const item = {
              category: null,
              category_id: null,
    ...
    toggleItemStatus(){
      this.$refs.changeStatusMaRef.toggleStatusDialog(this.currentTab.selectedItems[0]);
    },
        ...
        if (result.status == 201) {
          ...
          if(this.item_code_tab=='all'){
            this.tableRecords = this.tableRecords.map((i) => {
              ...
              this.tableRecords = this.tableRecords.filter((data)=>data.id!=item.id);
    ...
    async refreshMissedAds() {
      this.refresh_error = 0;
      try {
        if (this.refreshDates != null) {
          this.menu1 = false;
          this.showMissedAds = true;
          let data = { dates: this.refreshDates };
          let url = `advertisement/connection/refresh-platform`;
          let count = await this.$axios.post(url, data);

          if (count.data == 0) {
            this.showMissedAds = false;
            this.$toasterNA("red", this.$tr("no_active_ads_to_refresh"));
            return;
          } else {
            this.$toasterNA(
